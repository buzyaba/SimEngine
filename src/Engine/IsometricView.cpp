...
static glm::vec3 sphereCam;
static double lastX;
static double lastY;
static bool rightMouseClick;

IsometricView::IsometricView(const std::uint32_t& screenWidth, const std::uint32_t& screenHeight, 
                            const std::string winName, bool _visible): WindowManager(screenWidth, 
                                                                                    screenHeight,
                                                                                    winName,
                                                                                    _visible)
{
...
void IsometricView::updateKeyboard(GLFWwindow* window, int key, int scancode, int action, int mods){
...
void IsometricView::updateMouseButtons(GLFWwindow* window, int button, int action, int mods) {
...
void IsometricView::updateMouse(GLFWwindow* window, double xpos, double ypos){
	if (rightMouseClick) {
		...
		if(sphereCam[1] >= 85)
				sphereCam[1] = 85;
		prevCamFront += prevCamPos - IsometricView::sphericalToCartesian(sphereCam);
		prevCamPos = IsometricView::sphericalToCartesian(sphereCam);
...
void IsometricView::updateScroll(GLFWwindow* window, double xoffset, double yoffset) {
	glm::vec3 prevCamPos = Renderer::getCamera()->getCameraPosition();	
	glm::vec3 prevCamFront = Renderer::getCamera()->getCameraFront();	
	glm::vec3 offsetCam = prevCamPos + prevCamFront; 
	prevCamPos -= offsetCam;
...
glm::vec3 IsometricView::cartesianToSpherical(glm::vec3 vec){
	glm::vec3 spherical;
...
glm::vec3 IsometricView::sphericalToCartesian(glm::vec3 vec){
	glm::vec3 cartesian;
...
void IsometricView::initMousePosition(GLfloat x, GLfloat y){
    lastX = x;
    lastY = y;
}
