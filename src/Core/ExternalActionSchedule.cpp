...
std::map<std::string, IProperties*>& TExternalActionSchedule::ChangeProperties(
    int objectIndex,
    std::map<std::string, IProperties*>& properties,
    std::size_t time) {
    for (auto& elem : objectPropertyIntervals[objectIndex]) {
        if (properties[elem.first]) {
            auto pointIndex = elem.second.GetTimePointIndex(time);
            if (pointIndex == -1)
                continue;
            std::map<std::string, double>& tmp = elem.second.value[pointIndex];
            ...
            for (int i = 0; i < objects.size(); i++) {
                for (auto& elem : objects[i]->GetProperties())
                    if (elem.first != "Name" && elem.first != "Object")
                        objectPropertyIntervals[i][elem.first].SetProperty(*elem.second,
                                                                           intervalCount,
                                                                           timePoint);
            }
        }
...
TExternalActionSchedule::TExternalActionSchedule(std::vector<TObjectOfObservation*> _objects,
                                                 std::string xmlName) {
