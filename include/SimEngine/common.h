...
template<>
std::vector<double> ParseString<double>(std::string s, std::string delimiter) {
...
std::vector<std::vector<double>> 
    ParseString<std::vector<double>>(std::string s, std::string delimiter) {
        const std::regex re("\\(([0-9,\\.]+)\\)");
        ...
        std::sregex_iterator end;
         for(; iter != end; ++iter) {
            for(std::size_t i = 1; i < iter->size(); ++i) {
                res.push_back(ParseString<double>((*iter)[i], delimiter));
            }
         }
        return res;
}
